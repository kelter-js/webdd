...
const getFlagByItemId = (baseId: BASE_ITEMS_ID) => {
  if (baseId === BASE_ITEMS_ID.CHAOS_CHALICE_TIER_1) {
    return FLAGS.CHAOS_CHALICE_REVEICED;
  }

  return null;
};

export const acquireArtifact =
  (set: StoreSet) =>
  (resourceType: RESOURCES | null, baseId?: BASE_ITEMS_ID) => {
    set((state) => {
      const stateCopy = { ...state, player: { ...state.player } };

      if (!resourceType && baseId) {
        const item: MemoizedItem = [baseId, v4()];
        stateCopy.player.inventory_memoized.push(item);
        const flag = getFlagByItemId(baseId);

        if (flag) {
          stateCopy.player.flags.push(flag);
        }
